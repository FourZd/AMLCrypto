    ...
    async def main():
        ...
        async with redis_pool.get_redis() as redis_client:
            try:
                ...
                while True:
                    try:
                        location_service: ILocationService = Container().location_service()
                        oldest_locations: List[LocationDTO] = (
                            await location_service.get_locations_without_address(limit=100)
                        )
...
def export_activity_stats(self):
    async def main():
        ...
        async with redis_pool.get_redis() as redis_client:
            try:
                have_lock = await redis_client.set(LOCK_KEY, "1", nx=True, ex=LOCK_TIMEOUT)
                if not have_lock:
                    logger.info("Задача уже выполняется другим воркером. Пропуск выполнения.")
                    return
    ...
    asyncio.run(main())  # Use asyncio.run for better event loop management
